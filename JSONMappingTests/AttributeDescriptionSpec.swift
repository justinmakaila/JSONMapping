...
class AttributeDescriptionSpec: QuickSpec {
    override func spec() {
        ...
        it ("can transform boolean values") {
            ...
            let boolValue = booleanAttribute.value(usingRemoteValue: remoteValue) as? Bool
            
            expect(boolValue).toNot(beNil())
            expect(boolValue).to(beFalse())
            
            let numberOneValue = booleanAttribute.value(usingRemoteValue: 1) as? Bool
            
            expect(numberOneValue).toNot(beNil())
            expect(numberOneValue).to(beTrue())
            
            let zeroValue = booleanAttribute.value(usingRemoteValue: 0) as? Bool
            
            expect(zeroValue).toNot(beNil())
            expect(zeroValue).to(beFalse())
            
            let randomValue = booleanAttribute.value(usingRemoteValue: arc4random_uniform(100)) as? Bool
            
            expect(randomValue).toNot(beNil())
            expect(randomValue).to(beTrue())
            
            /// !!!: I'm not sure if this is desired, but it's definitely happening.
            let stringValue = booleanAttribute.value(usingRemoteValue: "") as? Bool
            expect(stringValue).to(beNil())
            expect(stringValue).to(beFalsy())
            
            /// !!!: I'm not sure if this is desired, but it's definitely happening.
            let trueStringValue = booleanAttribute.value(usingRemoteValue: "true") as? Bool
            expect(trueStringValue).to(beNil())
            expect(trueStringValue).to(beFalsy())
            
            /// !!!: I'm not sure if this is desired, but it's definitely happening.
            let falseStringValue = booleanAttribute.value(usingRemoteValue: "false") as? Bool
            expect(falseStringValue).to(beNil())
            expect(falseStringValue).to(beFalsy())
            
            /// !!!: I'm not sure if this is desired, but it's definitely happening.
            let randomStringValue = booleanAttribute.value(usingRemoteValue: "alshegwe") as? Bool
        ...
        it ("can transform a string to a 16-bit integer") {
            ...
            let stringValue = intAttribute.value(usingRemoteValue: "12") as? Int16
        ...
        it ("can transform a string to a 32-bit integer") {
            ...
            let stringValue = intAttribute.value(usingRemoteValue: "12") as? Int32
        ...
        it ("can transform a string to a 64-bit integer") {
            ...
            let stringValue = intAttribute.value(usingRemoteValue: "12") as? Int64
        ...
        it ("can transform a date string to a date") {
            ...
            let stringValue = dateAttribute.value(usingRemoteValue: birthdateString, dateFormatter: customDateFormatter)
